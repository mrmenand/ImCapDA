...
from models.ImCapFusionDA import ImCapFusionDAC
...
from utils.val_test import test_acc_imcapfusionda
...
def train(source_loader, target_train_loader, target_test_loader, model, optimizer, scheduler, args):
    best_acc = 0
    # Zero short Acc.
    _, acc_zero_shot_clip, _ = test_acc_imcapfusionda(model, target_test_loader, args)
    ...
    for global_step in range(args.max_iter):
        ...
        with autocast("cuda"):
            image_features, blip_text_features, logits_clip, logits_fc = model(inputs, blip_caption)

            feats_src, feats_tgt = image_features.chunk(2, dim=0)
            outputs_src, outputs_tgt = logits_fc.chunk(2, dim=0)
            outputs_src_clip, outputs_tgt_clip = logits_clip.chunk(2, dim=0)
            ...
            contrastive_loss = vlm_text_contrastive_loss(image_features, blip_text_features)
        ...
        # loss = classifier_loss + args.alpha * loss_u + args.beta * contrastive_loss
        #loss =  classifier_loss + transfer_loss + args.alpha * loss_u
        ...
        if (global_step + 1) % args.eval_every == 0:
            ...
            test_acc, test_acc_clip, test_loss = test_acc_imcapfusionda(model, target_test_loader, args)
            info += f', test_loss {test_loss:.2f}, test_acc_fc: {test_acc:.2f},  test_acc_clip: {test_acc_clip:.2f}'

            if best_acc < test_acc:
                best_acc = test_acc

            logging.info(info);tqdm.write(info)
            time.sleep(1)
...
def get_parser():
    ...
    parser.add_argument('--fuse_mode', default=True, action='store_true')
...
def main():
    ...
    model = ImCapFusionDAC(args).to(args.device)
